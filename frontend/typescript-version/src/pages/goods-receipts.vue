...
const handleConfirmReceipt = async () => {
  ...
  if (!confirm('Bạn có chắc chắn muốn xác nhận phiếu nhập này không? Thao tác này sẽ tăng tồn kho và cập nhật giá nhập.')) return
...
const handleCancelReceipt = async () => {
  if (!selectedReceipt.value) return
  if (!confirm('Bạn có chắc chắn muốn hủy phiếu nhập này không?')) return
      ...
      <VCard class="mb-6">
        ...
        <VCardText class="d-flex flex-column gap-3">
          <div class="d-flex align-start gap-2">
            <VIcon color="primary" icon="ri-checkbox-circle-line" class="mt-1" />
            <div>
        ...
        <VTable class="retail-table mb-4">
          <thead>
            <tr>
              ...
              <th>Mã hàng</th>
              <th>ĐVT</th>
              <th class="text-right" style="width: 150px;">Số lượng nhập</th>
              <th class="text-right" style="width: 200px;">Giá nhập (đ)</th>
  ...
  >
    <VCard v-if="selectedReceipt">
      <VCardTitle class="d-flex justify-between align-center">
        ...
        <VChip :color="statusColor(selectedReceipt.status)">
